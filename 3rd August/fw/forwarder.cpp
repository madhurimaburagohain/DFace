...
/*std::tuple <long int, long int, double> Forwarder ::CollectInfo()
{
        long int total_interest1, total_data1;
        ...
        return std::make_tuple(total_interest1, total_data1, isr);
}*/
...
std::tuple <long int, long int, double, int, long int > Forwarder ::CollectInfo()
{
 ...
 void Forwarder::TraceInfo1 ( ns3::Ptr<ns3::Node> node,ns3::Time next) {

  
  std::cout<< "\nHi\n";
  
 // if ( m_pit.size()>=400 *0.8)
 
 {
 
 //std::cout << Forwarder::prev << "\n";
        
        
      /*  if (prefix_mark_map[interestPrefix] >0)
        {
                prev=current;
                current= prefix_mark_map[interestPrefix];
                prefix_mark_map[interestPrefix]=0;
                
        }*/
  }
   
 ns3:: Simulator::Schedule (next, TraceInfo1,node, next);
...
Forwarder::onIncomingInterest(const FaceEndpoint& ingress, const Interest& interest)
{
   ...
   Name interestNamePrefix=interest.getName().getPrefix(-1);
   std::string interestPrefix= interestNamePrefix.toUri();
   ...
   if (str.compare (0, strlen (localhost_prefix), localhost_prefix )!=0) {
       ...
       /* if ( m_pit.size()>=400 *0.8){
        Forwarder::TraceInfo1 (node, ns3::MilliSeconds (500));
        }*/
        
        //ns3::Simulator::Schedule (ns3::MilliSeconds (500), TraceInfo1, node, ns3::MilliSeconds (500));
         if ( prefix_interest_map. find( interestPrefix ) ==prefix_interest_map.end()){
    
    
                        prefix_interest_map[interestPrefix]=1;
         }
         else{
                prefix_interest_map[interestPrefix]= prefix_interest_map[interestPrefix]+1;
   ...
   if ( m_pit.size()>=400 *0.8){
    ...
    if (prefix_mark_map[interestPrefix]>20)
                      {
                                
                              //  std::cout << " ***Congestion marking number " <<  prefix_mark_map[interestPrefix]<< "\n";
   ...
   /* track ISR for each prefixes..since unresponsive consumers will not reduce their Interest sending rate even after receiving exlicit marking*/
   
if(   ns3::Simulator::GetContext()==4) 
{
  ...
  //if (m_pit.size()>=800 *.8)
  {
             ...
             // std::cout << "\n Congestion marking number  =  " <<  prefix_mark_map[interestPrefix]<< "\t" << interestPrefix << "\n";
               
   
                if ( prefix_isr_map. find( interestPrefix ) !=prefix_isr_map.end()){
                
                        prefix_isr_map[interestPrefix]=(double)prefix_data_map[interestPrefix]/prefix_interest_map[interestPrefix];
                        
                      //if (prefix_isr_map[interestPrefix]<0.8){
                      
                      if (prefix_mark_map[interestPrefix]>50)
                      {
   ...
   //---------------------------------------------------
   int hop_count;
   auto nextHopTag = interest.getTag<lp::HopCountTag>();
   if (nextHopTag!=nullptr){
        hop_count= * nextHopTag;
        
       // std::cout << " hop_count " <<  hop_count << "\n";
        
        interest.setTag(make_shared<lp::HopCountTag>( hop_count+1));
        
        if ( hop_count==1)
        {
               // std::cout << "\n I am a consumer gateway router \t" <<  ns3::Simulator::GetContext() << "\n" ;
               
               
               const char consumer_prefix []= "/dst1/";
        std::string str= interest.getName().toUri() ;
   
       Name interestNamePrefix=interest.getName().getPrefix(-1);
       std::string interestPrefix= interestNamePrefix.toUri();
       if (str.compare (0, strlen (consumer_prefix ), consumer_prefix   )!=0) {
       int count=0;
                for (auto it=m_pit.begin(); it !=m_pit.end();it++) {
         
                //std::cout << "PIT entry name \t" << it-> getName() << "\n";
                
                const char doubt_prefix []= "/dst1/";
                
                std::string str5=it-> getName().getPrefix(-1).toUri();
                
                if (str5.compare (0, strlen (doubt_prefix), doubt_prefix )!=0) {
         ...
         if (count>= pit_size){
                ...
                //std::cout << "\n count and pit size = " << count << "\t" << pit_size << "\n";
...
void Forwarder::onReductionPITEntry()
{
         //std::cout << "\nwant to reduce number of pIT pentries for prefix 1\n";
         int count=0;
         isr=0.5;
         for (auto it=m_pit.begin(); it !=m_pit.end();it++) {
         ...
         pit_size=count* isr;
         //std::cout << "Number of PIT entries for prefix /dst1 = " << count << "\n";
...
Forwarder::onIncomingData(const FaceEndpoint& ingress, const Data& data)
{
  ...
  Name dataNamePrefix=data.getName().getPrefix(-1);
   std::string dataPrefix= dataNamePrefix.toUri();
   ...
   if (str.compare (0, strlen (localhost_prefix), localhost_prefix )!=0) {
        ...
        if ( prefix_data_map. find(dataPrefix ) ==prefix_data_map.end()){
   
    
                        prefix_data_map[dataPrefix]=1;
         }
         else {
                prefix_data_map[dataPrefix]= prefix_data_map[dataPrefix]+1;
   ...
   // if (m_pit.size()==800*0.8) 
    {
        
          if (data.getCongestionMark() > 0) { 
                        
              if ( prefix_mark_map. find(dataPrefix ) ==prefix_mark_map.end()){
  
    
                        prefix_mark_map[dataPrefix]=1;
                        
                        const auto now = time::steady_clock::now();
         }
         else {
                prefix_mark_map[dataPrefix]= prefix_mark_map[dataPrefix]+1;
   ...
   if (data.getCongestionMark() > 0) { 
        ...
        //getScheduler().schedule(duration, [=] { onInterestFinalize(outFaceId, pitEntry); });
        
        time::milliseconds  duration(1000);
        getScheduler().schedule(duration, [=] { onReductionPITEntry(); });
        
        
        // this->setRegressTimer(, pitEntry, 0_ms);
   }
  ...
  if (pitMatches.size() == 1) {
    ...
    pit::InRecordCollection::iterator lastExpiring =std::max_element(pitEntry->in_begin(), pitEntry->in_end(), &compare_InRecord_expiry);
