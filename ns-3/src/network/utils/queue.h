...
private:
  ...
  std::list<Ptr<Item> > m_packets1; 
    std::list<Ptr<Item> > m_packets2; 
...
Queue<Item>::DoEnqueue (ConstIterator pos, Ptr<Item> item)
{
 ...
 //------------------------------------- 
  ...
  inline uint32_t Time2CoDel (Time t){
        return (t.GetNanoSeconds () >> 10);
  }

        inline bool CoDelTimeAfter (uint32_t a, uint32_t b)
        {
...
static const int  CODEL_SHIFT1 = 10;
Time time = Simulator::Now ();
uint64_t ns = time.GetNanoSeconds ();
CoDelTimestampTag tag;
bool found = item->RemovePacketTag(tag);
Time delta = Simulator::Now() - tag.GetTxTime();
//uint32_t sojournTime = delta.GetNanoSeconds ()>> CODEL_SHIFT1;
...
if (CoDelTimeAfter(sojournTime, Time2CoDel(m_target))) {
        ...
        flag=1;
      // std::cout << "\nSojourn time is greater than threshold\t" <<  sojournTime  << "m-target1 " << m_target << "\n";
...
/*if((int)(sojournTime)-(int) (m_target1) >0){

        flag=1;
       std::cout << "\nSojourn time is greater than threshold\t" <<  sojournTime  << "m-target1 " << m_target1 << "\n";
}*/
