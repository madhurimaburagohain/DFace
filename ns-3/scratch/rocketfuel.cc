...
int main (int argc, char *argv[])
{
 ...
 NodeContainer consumers,  producers;
    ...
    while (consumers.size () <number_of_consumers)
    {
   ...
   while (producers.size () < number_of_producers)
    {
       Ptr<Node> node ;
   ...
   for (NodeContainer::Iterator node = producers.begin (); node != producers.end (); node++)
    {
        ...
        //producerHelper.SetPrefix("/good/"+to_string(i)+"/");
       
        

       // ndnGlobalRoutingHelper.AddOrigins ("/good/"+to_string(i)+"/",*node);
