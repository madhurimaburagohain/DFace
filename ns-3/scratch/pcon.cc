...
std::cout << "\n pit size \t" << size << "\n";
  ...
  f << now.ToDouble (Time::S)<< "\t" << (double)(interest)/10<< "\t" << (double) (data )/10<< "\t" << isr << "\t" << pit_size << "\t" << delay << "\n";
...
consumerHelper.SetPrefix("/dst1");
 consumerHelper.SetAttribute("LifeTime", StringValue("4s"));
  ApplicationContainer Aconsumer1= consumerHelper.Install(consumer1);
