...
//string file_name= string("Data_")+  node_name  + string(".txt");
  ...
  f << now.ToDouble (Time::S)<< "\t" << (double)(interest)/3<< "\t" << (double) (data )/3<< "\t" << isr << "\t" << pit_size << "\t" << delay << "\n";
...
main(int argc, char* argv[])
{
  ...
  topologyReader.SetFileName("src/ndnSIM/examples/topologies/topo-pcon_scen4.2_testing_queue.txt");
  topologyReader.Read();
...
ndnGlobalRoutingHelper.CalculateAllPossibleRoutes();
 // NodeContainer nc = NodeContainer::GetGlobal();
  ...
  for (uint32_t i=0;i<nc.GetN();i++){
	...
	Simulator::Schedule (Seconds (3), TraceInfo,  nc.Get (i), Seconds (3));
  }

  Simulator::Stop(Seconds(45.0)); //45
